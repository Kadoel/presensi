...
class TukarJadwalService extends BaseService
{
    ...
    protected function validasiTukarSlot(object $jadwalA, object $jadwalB): array
    {
        ...
        // no-op untuk tanggal sama + shift sama
        if (
            $tanggalA === $tanggalB &&
            (int) $jadwalA->shift_id === (int) $jadwalB->shift_id
        ) {
            return $this->hasilGagal([], 'Tukar jadwal ditolak karena kedua slot memiliki tanggal dan shift yang sama');
        }
    ...
    protected function logDbError(string $context): void
    {
        $dbError = $this->db->error();

        if (! empty($dbError['code']) || ! empty($dbError['message'])) {
            log_message('error', $context . ' | DB Error: ' . json_encode($dbError));
        }
    }
